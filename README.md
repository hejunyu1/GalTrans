...
只有测试内的确定性后端替身，没有真实模型、任务数据库、缓存、费用计算或翻译命令；通过校验
的结果仍只是提案，不会自动进入渲染、导出或启动边界。
...
具体像素、译文内容、字体、溢出或交互路线。当前仍没有接入译文输入命令或真实模型。结构化
任务规范和检查点可以严格序列化并恢复，但尚未实现 SQLite 持久化，因此不能把它描述成已经
具备进程崩溃后的原子落盘恢复。
