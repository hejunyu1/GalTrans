...
同时拒绝旧检查点覆盖新进度。当前只有测试内的确定性后端替身，没有真实模型、请求缓存、费用
计算或翻译命令；通过校验的结果仍不会自动进入渲染、导出或启动边界。
...
存储已经能原子保存任务、检查点和已接受提案，但尚未定义玩家工作区、缓存、审计、费用记录或
自动任务执行，因此不能把它描述成已经完成面向玩家的断点续传工作流。
