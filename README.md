...
未知请求，只对确定失败进行显式受控重试，无法消歧时要求人工处理。当前仍只有测试内的确定性
Provider 替身，没有真实模型、网络、凭据、费用计算或翻译命令；通过校验的结果不会自动进入
渲染、导出或启动边界。已完成任务现在可以在调用方同时提供原始提取记录和官方 SDK 交叉验证
映射时，再次核对来源、检查点与提案，并在纯内存中准备现有 Ren'Py 导出器可接收的翻译文件；
该内部接口不会调用 SDK、写出目录或启动游戏。已经过引擎校验的完整任务还可以生成关闭式、
...
定义玩家工作区、翻译记忆、审计、费用记录、真实模型进程或翻译命令。Provider 回执和查询协议
只由确定性替身验证；它定义了真实服务必须提供的幂等与消歧边界，但尚未证明任何实际服务的
幂等期限、查询能力或计费语义。已接受提案到 Ren'Py 内存文件的桥接也只是内部确定性 API，
没有工作区、人工审批或显式磁盘发布入口，因此还不能描述成完整的费用去重、玩家断点续传或
自动导出工作流。当前质量报告同样只存在于内存，只检查完整任务身份和“译文与原文未变化”；
它没有持久化复核状态、质量评分、术语一致性或语义正确性检查，也不会自动阻断或授权导出。
