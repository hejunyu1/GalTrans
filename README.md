...
V0.4.4 已把第一个真实 OpenAI 兼容 Provider、可恢复翻译状态、确定性质量检查、Ren'Py 渲染和
...
反编译脚本。新增的内部导入边界只接受调用方已确认有权本地处理、且使用官方普通 `RPA-3.0`
结构保存原始 `.rpy/.rpym` 的成品；它通过受限索引解析，把源码原始字节发布到原游戏之外的全新
source-only 快照，并记录归档、索引和逐文件 SHA-256 清单。未知或自定义归档、危险索引、仅有
`.rpyc/.rpymc` 的成品都会在写入前拒绝。该边界尚未接入工作台、CLI 或自动翻译，完整成品游戏
导入与 Windows 安装包仍未实现。固定 Python 后端和 CPython 运行时、标准库及所需模块一起冻结为
Tauri Windows sidecar，运行时不查找仓库 `.venv`、源码目录或 `PYTHONPATH`。
...
内部 `import_renpy_packaged_sources` 接口为 `packaged_requires_import` 增加一个严格关闭的 V0.4.4
子集：只读取官方普通 `RPA-3.0` 的有界 zlib/pickle 索引，受限反序列化器只允许普通容器和 Ren'Py
官方可回滚字典/列表的安全替代映射；只复制单段、可识别文本编码的 `.rpy/.rpym`，不复制资源、
编译脚本或已有翻译。调用方必须传入关闭式授权确认，输出必须是原游戏外尚不存在的新目录；所有
源码保持原始字节，并生成 `galtrans-import.json`。默认最多检查 128 个归档，单个归档最多 8 GiB、
总归档最多 32 GiB；每个索引最多 32 MiB、50,000 项，单个源码最多 16 MiB、总源码最多 128 MiB。
任何路径穿越、Windows 路径冲突、范围越界、归档变化、未知头部或危险 pickle 都会清理暂存目录
并停止。该快照不是游戏副本，当前界面和自动流程也不会自行调用它。
...
或自动启动。现代 Tauri 工作台和 Tkinter 回退界面都只连接现有 source-only 自动流程；V0.4.4
的普通 RPA 源码快照导入仍是未接入界面的内部边界，不能处理仅有编译脚本的普通成品。Tauri
工作台已使用包含 CPython 与所需模块的固定 Windows sidecar，不再依赖仓库 `.venv` 运行，但尚未
提供安装器、自动发现 SDK 或完整支持普通成品游戏。首条质量检查只覆盖“译文
