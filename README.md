...
验证器和已接受提案重新计算，不能用调用方自报的 `clear` 覆盖需要复核的结果。
...
自动导出工作流。当前质量检查只覆盖完整任务身份和“译文与原文未变化”；完成任务可以显式
持久化这份报告，但仍没有人工批准/驳回状态、质量评分、术语一致性或语义正确性检查，也不会
自动阻断或授权导出。SQLite v1 至 v3 继续明确拒绝，不进行自动迁移。
