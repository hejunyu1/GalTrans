...
落盘后避免崩溃重放再次调用同一后端配置。当前仍只有测试内的确定性后端替身，没有真实模型、
在途请求查询、费用计算或翻译命令；通过校验的结果不会自动进入渲染、导出或启动边界。
...
定义玩家工作区、翻译记忆、审计、费用记录、真实模型进程或翻译命令。结果缓存只覆盖已收到并
验证的响应，不能判断服务端是否执行了本地尚未收到的请求，因此还不能描述成完整的费用去重或
面向玩家的断点续传工作流。
